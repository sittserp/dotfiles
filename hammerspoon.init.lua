hs.hotkey.bind({}, "F6", function()
  hs.eventtap.keyStrokes("051990")
  n = os.time()
  while os.time() - n < 1 do
    -- wait for 1 second
  end
...
This hidutil command will set:
  ...
  // The three below are for windows keyboards
...
```
--]]
-- Deprecated: Use Fn as numlock now
...
--[[
-- Custom Mode (called a modal): Number Mode
-- Fn is numlock
-- Custom modal environment
local numberMode = hs.hotkey.modal.new()
local isInNumberMode = false
-- Define what happens when you enter the modal
numberMode.entered = function()
  isInNumberMode = true
end
-- Define what happens when you exit the modal
numberMode.exited = function()
  isInNumberMode = false
end
-- Bind keys inside the modal
for num, key in pairs(numKeyMap) do
  numberMode:bind({}, key, function()
    hs.eventtap.keyStrokes(tostring(num))
  end)
end
-- Watch for fn key presses
fnWatcher = hs.eventtap.new({ hs.eventtap.event.types.flagsChanged }, function(event)
  local flags = event:getFlags()
  if flags.fn and not isInNumberMode then
    numberMode:enter()
  elseif flags.fn and isInNumberMode then
    numberMode:exit()
  end
  return false
end):start()
